...
local function lock_group_links(msg, data, target)
  ...
  if group_link_lock == '🔐' then
    return '<code>ارسال لینک در سوپر گروه از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['lock_link'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>ارسال لینک در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_links(msg, data, target)
  ...
  local group_link_lock = data[tostring(target)]['settings']['lock_link']
  if group_link_lock == '🔓' then
    return '<code>ارسال لینک در سوپر گروه از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['lock_link'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>ارسال لینک در سوپر گروه مجاز ش</code>د'
  end
end
...
local function lock_group_all(msg, data, target)
  ...
  if group_all_lock == '🔐' then
    return '<code>تمامی قفل های تنظیمات از قبل فعال بودند</code>'
  else
    data[tostring(target)]['settings']['all'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>تمامی قفل های تنظیمات فعال شدند</code>'
  end
end
...
local function unlock_group_all(msg, data, target)
  ...
  local group_all_lock = data[tostring(target)]['settings']['all']
  if group_all_lock == '🔓' then
    return '<code>تمامی قفل های تنظیمات از قبل مجاز بودند</code>'
  else
    data[tostring(target)]['settings']['all'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>تمامی قفل های تنظیمات مجاز شدند</code>'
  end
end
...
local function lock_group_normal(msg, data, target)
  ...
  if group_normal_lock == '🔐' then
    return '<code>سویچ نرمال از قبل فعال بود</code>'
  else
    data[tostring(target)]['settings']['normal'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>سویچ نرمال هم اکنون در سوپر گروه فعال شد</code>'
  end
end
...
local function unlock_group_normal(msg, data, target)
  ...
  local group_normal_lock = data[tostring(target)]['settings']['normal']
  if group_normal_lock == '🔓' then
    return '<code>سویچ نرمال از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['normal'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>سویچ نرمال هم اکنون در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_family(msg, data, target)
  ...
  if group_family_lock == '🔐' then
    return '<code>سویچ خانوادگی از قبل فعال بود</code>'
  else
    data[tostring(target)]['settings']['family'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>سویچ خانوادگی هم اکنون در سوپر گروه فعال شد</code>'
  end
end
...
local function unlock_group_family(msg, data, target)
  ...
  local group_family_lock = data[tostring(target)]['settings']['family']
  if group_family_lock == '🔓' then
    return '<code>سویچ خانوادگی از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['family'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>سویچ خانوادگی هم اکنون در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_etehad(msg, data, target)
  ...
  if group_etehad_lock == '🔐' then
    return '<code>سویچ اتحاد از قبل فعال بود</code>'
  else
    data[tostring(target)]['settings']['etehad'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>سویچ اتحاد هم اکنون در سوپر گروه فعال شد</code>'
  end
end
...
local function unlock_group_etehad(msg, data, target)
  ...
  local group_etehad_lock = data[tostring(target)]['settings']['etehad']
  if group_etehad_lock == '🔓' then
    return '<code>سویچ اتحاد از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['etehad'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>سویچ اتحادهم اکنون در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_leave(msg, data, target)
  ...
  if group_leave_lock == '🔐' then
    return '<code>خروج از گروه و بازگشت دوباره از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['leave'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>خروج از گروه و بازگشت دوباره قفل شد</code>'
  end
end
...
local function unlock_group_leave(msg, data, target)
  ...
  local group_leave_lock = data[tostring(target)]['settings']['leave']
  if group_leave_lock == '🔓' then
    return '<code>خروج از گروه و بازگشت دوباره از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['leave'] = ''
    save_data(_config.moderation.data, data)
    return '<code>خروج از گروه و بازگشت دوباره مجاز شد</code>'
  end
end
...
local function lock_group_operator(msg, data, target)
  ...
  if group_operator_lock == '🔐' then
    return '<code>ارسال نام اوپراتور ها از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['operator'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>ارسال نام اپراتور ها هم اکنون قفل شد</code>'
  end
end
...
local function unlock_group_operator(msg, data, target)
  ...
  local group_operator_lock = data[tostring(target)]['settings']['operator']
  if group_operator_lock == '🔓' then
    return '<code>ارسال نام اپراتور ها از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['operator'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>ارسال نام اپراتور ها هم اکنون مجاز شد</code>'
  end
end
...
local function lock_group_reply(msg, data, target)
  ...
  if group_reply_lock == '🔐' then
    return '<code>پاسخ به پیام به ریپلای از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['reply'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>پاسخ به پیام با ریپلای قفل شد</code>'
  end
end
...
local function unlock_group_reply(msg, data, target)
  ...
  local group_reply_lock = data[tostring(target)]['settings']['reply']
  if group_reply_lock == '🔐' then
    return '<code>پاسخ به پیام با ریپلای از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['reply'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>پاسخ به پبام با ریپلای مجاز شد</code>'
  end
end
...
local function lock_group_username(msg, data, target)
  ...
  if group_username_lock == '🔐' then
    return '<code>ارسال یوزرنیم از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['username'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>ارسال یوزرنیم در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_username(msg, data, target)
  ...
  local group_username_lock = data[tostring(target)]['settings']['username']
  if group_username_lock == '🔓' then
    return '<code>ارسال یوزرنیم در سوپر گروه از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['username'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>ارسال یوزرنیم در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_media(msg, data, target)
  ...
  if group_media_lock == '🔐' then
    return '<code>رسانه ها در سوپر گروه از قبل قفل بودند</code>'
  else
    data[tostring(target)]['settings']['media'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>رسانه ها در سوپر گروه قفل شدند</code>'
  end
end
...
local function unlock_group_media(msg, data, target)
  ...
  local group_media_lock = data[tostring(target)]['settings']['media']
  if group_media_lock == '🔓' then
    return '<code>رسانه ها در سوپر گروه از قبل مجاز بودند</code>'
  else
    data[tostring(target)]['settings']['media'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>رسانه ها در سوپر گروه مجاز شدند</code>'
  end
end
...
local function lock_group_fosh(msg, data, target)
  ...
  if group_fosh_lock == '🔐' then
    return '<code>فحاشی در سوپر گروه از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['fosh'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>فحاشی در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_fosh(msg, data, target)
  ...
  local group_fosh_lock = data[tostring(target)]['settings']['fosh']
  if group_fosh_lock == '🔓' then
    return '<code>فحاشی در سوپر گروه از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['fosh'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>فحاشی در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_photo(msg, data, target)
  ...
  if group_photo_lock == '🔐' then
    return '<code>ارسال تصویر از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['photo'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>ارسال تصویر در سوپر گروه قفل شد </code>'
  end
end
...
local function unlock_group_photo(msg, data, target)
  ...
  local group_photo_lock = data[tostring(target)]['settings']['photo']
  if group_photo_lock == '🔓' then
    return '<code>ارسال تصویر از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['photo'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>ارسال تصویر در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_video(msg, data, target)
  ...
  if group_video_lock == '🔐' then
    return '<code>ارسال ویدیو از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['video'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>ارسال ویدیو در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_video(msg, data, target)
  ...
  local group_video_lock = data[tostring(target)]['settings']['video']
  if group_video_lock == '🔓' then
    return '<code>ارسال ویدیو از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['video'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>ارسال ویدیو در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_audio(msg, data, target)
  ...
  if group_audio_lock == '🔐' then
    return '<code>ارسال موزیک_ویس از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['audio'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>ارسال موزیک_ویس در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_audio(msg, data, target)
  ...
  local group_audio_lock = data[tostring(target)]['settings']['audio']
  if group_audio_lock == '🔓' then
    return '<code>ارسال موزیک_ویس از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['audio'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>ارسال موزیک_ویس در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_gif(msg, data, target)
  ...
  if group_gif_lock == '🔐' then
    return '<code>ارسال تصاویر متحرک از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['gif'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>ارسال تصاویر متحرک در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_gif(msg, data, target)
  ...
  local group_gif_lock = data[tostring(target)]['settings']['gif']
  if group_gif_lock == '🔓' then
    return '<code>ارسال تصاویر متحرک از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['gif'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>ارسال تصاویر متحرک در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_website(msg, data, target)
  ...
  if group_website_lock == '🔐' then
    return '<code>ارسال وبسایت های اینترنتی از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['website'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>ارسال وبسایت های اینترنتی در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_website(msg, data, target)
  ...
  local group_website_lock = data[tostring(target)]['settings']['website']
  if group_website_lock == '🔓' then
    return '<code>ارسال وبسایت های اینترنتی از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['website'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>ارسال وبسایت های اینترنتی در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_chat(msg, data, target)
  ...
  if group_chat_lock == '🔐' then
    return '<code>چت کردن در سوپر گروه از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['chat'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>چت کردن در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_chat(msg, data, target)
  ...
  local group_chat_lock = data[tostring(target)]['settings']['chat']
  if group_chat_lock == '🔓' then
    return '<code>چت کردن در سوپر گروه از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['chat'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>چت کردن در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_chatall(msg, data, target)
  ...
  if group_chatall_lock == '🔐' then
    return '<code>ارسال هر نوع چیز از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['chatall'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>ارسال هر نوع چیز در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_chatall(msg, data, target)
  ...
  local group_chatall_lock = data[tostring(target)]['settings']['chatall']
  if group_chatall_lock == '🔓' then
    return '<code>ارسال هر نوع چیز از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['chatall'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>ارسال هر نوع چیز در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_poker(msg, data, target)
  ...
  if group_poker_lock == '🔐' then
    return '<code>ارسال پوکر از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['poker'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>ارسال پوکر در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_poker(msg, data, target)
  ...
  if group_poker_lock == '🔓' then
    return '<code>ارسال پوکر از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['poker'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>ارسال پوکر در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_join(msg, data, target)
  ...
  if group_join_lock == '🔐' then
    return '<code>ورود در سوپر گروه از قبل ممنوع بود</code>'
  else
    data[tostring(target)]['settings']['join'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>ورود در سوپر گروه با لینک ممنوع شد</code>'
  end
end
...
local function unlock_group_join(msg, data, target)
  ...
  local group_join_lock = data[tostring(target)]['settings']['join']
  if group_join_lock == '🔓' then
    return '<code>ورود در سوپر گروه از قبل ازاد بود</code>'
  else
    data[tostring(target)]['settings']['join'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>ورود در سوپر گروه ازاد شد</code>'
  end
end
...
local function lock_group_fwd(msg, data, target)
  ...
  if group_fwd_lock == '🔐' then
    return '<code>فروارد کردن از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['fwd'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>فروارد کردن در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_fwd(msg, data, target)
  ...
  local group_fwd_lock = data[tostring(target)]['settings']['fwd']
  if group_fwd_lock == '🔓' then
    return '<code>فروارد کردن از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['fwd'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>فروارد کردن در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_english(msg, data, target)
  ...
  if group_english_lock == '🔐' then
    return '<code>زبان انگلیسی از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['english'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>زبان انگلیسی در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_english(msg, data, target)
  ...
  local group_english_lock = data[tostring(target)]['settings']['english']
  if group_english_lock == '🔓' then
    return '<code>زبان انگلیسی از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['english'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>زبان انگلیسی در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_emoji(msg, data, target)
  ...
  if group_emoji_lock == '🔐' then
    return '<code>ارسال اموجی از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['emoji'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>ارسال اموجی در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_emoji(msg, data, target)
  ...
  local group_emoji_lock = data[tostring(target)]['settings']['emoji']
  if group_emoji_lock == '🔓' then
    return '<code>ارسال اموجی از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['emoji'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>ارسال اموجی در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_tag(msg, data, target)
  ...
  if group_tag_lock == '🔐' then
    return '<code>ارسال تگ از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['tag'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>ارسال تگ در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_tag(msg, data, target)
  ...
  local group_tag_lock = data[tostring(target)]['settings']['tag']
  if group_tag_lock == '🔓' then
    return '<code>ارسال تگ از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['tag'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>ارسال تگ در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if group_spam_lock == '🔐' then
    return '<code>اسپم در سوپر گروه از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['lock_spam'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>اسپم در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_spam(msg, data, target)
  ...
  local group_spam_lock = data[tostring(target)]['settings']['lock_spam']
  if group_spam_lock == '🔓' then
    return '<code>اسپم در سوپر گروه از قبل مجاز شد</code>'
  else
    data[tostring(target)]['settings']['lock_spam'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>اسپم در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_flood(msg, data, target)
  ...
  if group_flood_lock == '🔐' then
    return '<code>فلودینگ در سوپر گروه از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['flood'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>فلودینگ در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_flood(msg, data, target)
  ...
  local group_flood_lock = data[tostring(target)]['settings']['flood']
  if group_flood_lock == '🔓' then
    return '<code>فلودینگ در سوپر گروه از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['flood'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>فلودینگ در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == '🔐' then
    return '<code>عربی/فارسی در سوپر گروه از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['lock_arabic'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>عربی/فارسی در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_arabic(msg, data, target)
  ...
  local group_arabic_lock = data[tostring(target)]['settings']['lock_arabic']
  if group_arabic_lock == '🔓' then
    return '<code>عربی/فارسی در سوپر گروه از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['lock_arabic'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>عربی/فارسی در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == '🔐' then
    return '<code>کاربران گروه از قبل قفل بودند</code>'
  else
    data[tostring(target)]['settings']['lock_member'] = '🔐'
    save_data(_config.moderation.data, data)
  end
  return '<code>کاربران گروه در سوپر گروه قفل شدند</code>'
end
...
local function unlock_group_membermod(msg, data, target)
  ...
  local group_member_lock = data[tostring(target)]['settings']['lock_member']
  if group_member_lock == '🔓' then
    return '<code>کاربران گروه از قبل مجاز بودند</code>'
  else
    data[tostring(target)]['settings']['lock_member'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>کاربران گروه در سوپر گروه مجاز شدند</code>'
  end
end
...
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == '🔐' then
    return '<code>ار تی ال در سوپر گروه از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['lock_rtl'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>ار تی ال در سوپر گروه قفل شد </code>'
  end
end
...
local function unlock_group_rtl(msg, data, target)
  ...
  local group_rtl_lock = data[tostring(target)]['settings']['lock_rtl']
  if group_rtl_lock == '🔓' then
    return '<code>ار تی ال در سوپر گروه از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['lock_rtl'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>ار تی ال در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == '🔐' then
    return '<code>نمایش ورود و خروج از سوپر گروه  از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>نمایش ورود و خروج از سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_tgservice(msg, data, target)
  ...
  local group_tgservice_lock = data[tostring(target)]['settings']['lock_tgservice']
  if group_tgservice_lock == '🔓' then
    return '<code>نمایش ورود و خروج از سوپر گروه از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>نمایش ورود و خروج از سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == '🔐' then
    return '<code>ارسال استیکر در سوپر گروه از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['lock_sticker'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>ارسال استیکر در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_sticker(msg, data, target)
  ...
  local group_sticker_lock = data[tostring(target)]['settings']['lock_sticker']
  if group_sticker_lock == '🔓' then
    return '<code>ارسال استیکر در سوپر گروه از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['lock_sticker'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>ارسال استیکر در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_bots(msg, data, target)
  ...
  if group_bots_lock == '🔐' then
    return '<code>دعوت کردن ربات از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['lock_bots'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>دعوت کردن ربات در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_bots(msg, data, target)
  ...
  local group_bots_lock = data[tostring(target)]['settings']['lock_bots']
  if group_bots_lock == '🔓' then
    return '<code>دعوت کردن ربات از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['lock_bots'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>دعوت کردن ربات در سوپر گروه مجاز شد</code>'
  end
end
...
local function lock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == '🔐' then
    return '<code>اشتراک گذاری شماره از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['lock_contacts'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>اشتراک گذاری شماره در سوپر گروه قفل شد</code>'
  end
end
...
local function unlock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == '🔓' then
    return '<code>اشتراک گذاری شماره از قبل مجاز بود</code>'
  else
    data[tostring(target)]['settings']['lock_contacts'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>اشتراک گذاری شماره در سوپر گروه مجاز شد</code>'
  end
end
...
local function enable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == '🔐' then
    return '<code>تنظیمات سخت گیرانه از قبل قفل بود</code>'
  else
    data[tostring(target)]['settings']['strict'] = '🔐'
    save_data(_config.moderation.data, data)
    return '<code>تنظیمات سخت گیرانه فعال شد</code>'
  end
end
...
local function disable_strict_rules(msg, data, target)
  ...
  local group_strict_lock = data[tostring(target)]['settings']['strict']
  if group_strict_lock == '🔓' then
    return '<code>تنظیمات سخت گیرانه از قبل مجاز شد</code>'
  else
    data[tostring(target)]['settings']['strict'] = '🔓'
    save_data(_config.moderation.data, data)
    return '<code>تنظیمات سخت گیرانه مجاز شد</code>'
  end
end
...
local function lock_group_welcome(msg, data, target)
  ...
  if welcoms == '✅' then
    return '<code>پیام خوش امد گویی فعال است</code>'
  else
    data[tostring(target)]['welcome'] = '✅'
    save_data(_config.moderation.data, data)
    return '<code>پیام خوش امد گویی فعال شد\nبرای تغییر این پیام از دستور زیر استفاده کنید\n/set welcome <welcomemsg></code>'
  end
end

local function unlock_group_welcome(msg, data, target)
      if not is_momod(msg) then
        return "<code>شما مدیر گروه نیستید</code>"
      end
  local welcoms = data[tostring(target)]['welcome']
  if welcoms == '❌' then
    return '<code>پیام خوش امد گویی غیر فعال است</code>'
  else
    data[tostring(target)]['welcome'] = '❌'
    save_data(_config.moderation.data, data)
    return '<code>پیام خوش امد گویی غیر فعال شد</code>'
  end
end
...
local function set_rulesmod(msg, data, target)
  ...
  save_data(_config.moderation.data, data)
  return '<code>قوانین گروه ثبت شد</code>'
end
...
local function get_rules(msg, data)
  ...
  if not data[tostring(msg.to.id)][data_cat] then
    return '<code>قوانین گروه در دسترس نیست</code>'
  end
...
function show_supergroup_settingsmod(msg, target)
  ...
  local settings = data[tostring(target)]['settings']
      local text = "\n<code>♻تنظیمات سوپر گروه♻:\n\n🔵{قفل ارسال لینک گروه ها {لینک: "..settings.lock_link.."\n🔴{قفل ارسال تگ ها {تگ "..settings.tag.."\n🔵{قفل ارسال ادرس سایت ها {وبسایت: "..settings.website.."\n🔴{قفل ارسال اموجی ها {اموجی: "..settings.emoji.."\n🔵قفل صحبت به زبان انگلیسی انگلیسی: "..settings.english.."\n🔴{قفل ارسال تصاویر {تصاویر: "..settings.photo.."\n🔵{قفل ارسال ویدیو {فیلم: "..settings.video.."\n🔴قفل ارسال موزیک/ویس {موزیک: "..settings.audio.."\n🔵{قفل ارسال تصاویر متحرک {گیف: "..settings.gif.."\n🔴{قفل ارسال پوکر فیس {پوکر: "..settings.poker.."\n🔵{قفل ارسال متون طولانی {فلود: "..settings.flood.."\n🌎مقدار حساسیت به پیام مکرر: "..NUM_MSG_MAX.."\n🔴قفل ارسال پیام مکرر }{اسپم: "..settings.lock_spam.."\n: "..settings.lock_arabic.."\n🚨قفل کاربران: "..settings.lock_member.."\n♻{قفل راست به چت {ار تی ال: "..settings.lock_rtl.."\n🚨قفل ورود وخروج افراد : "..settings.lock_tgservice.."\n🚨قفل استیکر: "..settings.lock_sticker.."\🔽قفل فروارد کردن مطالب {فرارد: "..settings.fwd.."\n🔽{قفل ریپلای کردن پیام {ریپلای: "..settings.reply.."\n🌐{قفل چت در سوپر گروه {چت: "..settings.chat.."\n🌐قفل چت ها در سوپر گروه {چت ها : "..settings.chatall.."\n{قفل ورود کاربران جدید  {ورود: "..settings.join.."\n{قفل ارسال یوزرنیم {یوزرنیم: "..settings.username.."\n{قفل ارسال  رسانه ها {رسانه: "..settings.media.."\n{قفل فحاشی کردن در سوپر گروه {فحش: "..settings.fosh.."\nقفل خروج کاربران {خروج: "..settings.leave.."\nقفل اشتراک گذاری شماره: "..settings.lock_contacts.."\n{قفل دعوت کردن ربات  {ربات : "..bots_protection.."\nقفل سویچ مدل اتحاد {اتحاد: "..settings.etehad.."\n{قفل سویچ مدل نرمال {نرمال: "..settings.normal.."\n{قفل سویچ مدل خانوادگی {خانوادگی: "..settings.family.."\nقفل تمامی لاک های تنظیمات {همه: "..settings.all.."\nایدی شما :\n🌟مدل سوپر گروه: "..gp_type.."\n⚫عمومی بودن  : "..settings.public.."\n🚫تنظیمات سخت گیر : "..settings.strict.."\nپیام خوش آمد گویی : "..Welcome.."\nتاریخ انقضای گروه: "..Expiretime.."روز دیگر</code> "
...
function show_supergroup_mutes(msg, target)
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['mute_service'] then
			data[tostring(target)]['settings']['mute_service'] = '🔓'
		end
	end
	if data[tostring(target)]['settings'] then
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['mute_forward'] then
			data[tostring(target)]['settings']['mute_forward'] = '🔓'
		end
	end
	if data[tostring(target)]['settings'] then
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'id' then
			...
			elseif msg.text:match("@[%a%d]") then
				...
				return "\n<code>🚻ایدی شما: "..msg.from.id.."\n👥ایدی سوپر گروه: "..msg.to.id.."\n🌎یوزرنیم شما</code> : @"..msg.from.username
			end
		end
		...
		if matches[1] == 'clean' then
			...
			if matches[2] == 'username' and is_admin1(msg) then
				...
				channel_set_username(receiver, username, ok_username_cb, {receiver=receiver})
			                        ...
			                        if matches[2] == "members" and is_momod(msg) then
		...
		if matches[1] == 'mute' and is_momod(msg) then
			...
			local msg_type = 'Forward'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					data[tostring(msg.to.id)]['settings']['mute_forward'] = '🔐'
                    save_data(_config.moderation.data, data)
			...
			local msg_type = 'service'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type) 
					mute(chat_id, msg_type)
					data[tostring(msg.to.id)]['settings']['mute_service'] = '🔐'
                    save_data(_config.moderation.data, data)
